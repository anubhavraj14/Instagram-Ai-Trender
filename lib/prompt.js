...
// Schema for the Reel Editor: given a transcript with timestamps, produce an edit plan.
  ...
  "edits": [
    {
      ...
      "duration": 1.5,
      "scale": 1.25,
      "query": "2-4 concrete photographable keywords for a stock VIDEO (broll only) e.g. 'phone tripod filming', 'laptop editing timeline'",
      "text": "short on-screen text overlay in HINGLISH (textcard only, max 5 words)"
    }
  ]
}
...
- "zoom" = punch-in on the speaker for emphasis moments (hooks, punchlines, strong claims).
  scale 1.15-1.4, duration 0.6-1.6s. Use 3-6 zooms across the reel, on the most impactful lines.
- "broll" = full-screen stock clip replacing the speaker briefly when they describe something
  visual. ONLY when the moment is concretely filmable (apps, phones, desks, money, gym etc.).
  duration 1.5-3s. Max 4-5 broll moments — they must not interrupt a punchline.
- "textcard" = a bold text overlay emphasizing ONE key phrase/stat (e.g. "3x faster", "Stop doing this").
  duration 1.2-2s. Use sparingly, max 3.
- Timestamps (seconds) must come from the transcript's actual timing. Edits must not overlap.
- Order edits by start time. Leave breathing room — don't stack effects every 2 seconds.
`;

export function buildEditPlanPrompt(transcript, segments = [], duration = 0) {
  const segList = segments
    .map((s) => `[${s.start.toFixed(1)}-${s.end.toFixed(1)}s] ${s.text}`)
    .join("\n");
  return `You are a professional short-form video editor. A creator gave you a raw
talking-head video and its timestamped transcript. Produce an EDIT PLAN that makes
it engaging for Instagram Reels.

VIDEO DURATION: ${duration ? duration.toFixed(1) + "s" : "unknown"}

TRANSCRIPT SEGMENTS (with timestamps):
${segList || "(no segment timing available)"}

FULL TRANSCRIPT:
${transcript}

Think like a retention editor: zoom on the strongest lines, cut away to B-roll only
when the speaker references something visual, pop a text card on memorable stats.
Do NOT over-edit — clean beats over chaos.
