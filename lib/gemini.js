...
import { buildAnalysisPrompt, buildViralPrompt, buildPlanPrompt, DEFAULT_NICHE } from "./prompt.js";
import { fetchTrendSignals, fetchViralSignals } from "./sources.js";
...
export async function researchTrends(niche = DEFAULT_NICHE) {
  ...
  if (!signals.length) {
    throw new Error("Could not fetch any trend signals right now. Try again shortly.");
  }
...
export async function researchViral(niche = DEFAULT_NICHE) {
  ...
  const signals = await fetchViralSignals();
  if (!signals.length) {
    throw new Error("Could not fetch any viral signals right now. Try again shortly.");
  }
