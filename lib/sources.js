...
async function fetchNews(query, recency = "2d") {
  ...
  try {
    const res = await fetch(url, { headers: COMMON_HEADERS, signal: t.signal });
    if (!res.ok) return [];
...
// Signals about what makes Instagram CAROUSELS perform: hooks, layouts, design
// patterns, storytelling structures. Used to design an original, research-informed carousel.
const CAROUSEL_QUERIES = [
  "best Instagram carousel design tips",
  "Instagram carousel hooks that get saves",
  "high performing Instagram carousel examples",
  "Instagram carousel post structure viral",
  "Instagram carousel cover slide design",
  "Instagram carousel storytelling tips",
  "Instagram carousel layout ideas creators",
  "how to make Instagram carousels go viral",
];
const CAROUSEL_SUBREDDITS = [
  "InstagramMarketing",
  "InstagramGrowthTips",
  "socialmedia",
  "graphic_design",
...
export async function fetchCarouselSignals() {
  const results = await Promise.all([
    ...CAROUSEL_QUERIES.map((q) => fetchNews(q, "60d")),
    ...CAROUSEL_SUBREDDITS.map((s) => fetchReddit(s, "month")),
  ]);
  return dedupe(results.flat());
}
