...
const W = 720, H = 1280; // 720p keeps ffmpeg memory well under Render's 512MB cap

function runFfmpeg(args, label = "ffmpeg", onProgress = null) {
  return new Promise((resolve, reject) => {
    // -progress writes key=value status lines to stdout; out_time_us tracks position.
    if (onProgress) args = ["-progress", "pipe:1", "-nostats", ...args];
...
// Cheap single-stream pass: shrink any input (4K/1080p) to 720x1280/30fps so the
// heavy composite pass decodes small frames — the main OOM driver on small boxes.
export async function normalizeInput(input, output, onProgress = null) {
  await runFfmpeg([
    "-y", "-i", input, "-t", "95",
    "-vf", `scale=${W}:${H}:force_original_aspect_ratio=increase,crop=${W}:${H},setsar=1`,
    "-r", "30",
    "-c:v", "libx264", "-preset", "ultrafast", "-crf", "21",
    "-x264-params", "rc-lookahead=5:ref=1",
    "-threads", "2",
    "-pix_fmt", "yuv420p",
    "-c:a", "aac", "-b:a", "128k", "-ac", "2",
    output,
  ], "normalize input", onProgress);
}
...
Style: Cap,Arial,46,&H00FFFFFF,&H00FFFFFF,&H00000000,&H80000000,-1,0,0,0,100,100,0,0,1,4,0,2,40,40,110,1
Style: Card,Arial,72,&H0000FFFF,&H00FFFFFF,&H00000000,&H80000000,-1,0,0,0,100,100,0,0,1,5,0,8,40,40,95,1
...
export async function renderReel({ input, output, edits = [], brollFiles = new Map(), assPath, duration, onProgress = null }) {
  ...
  for (const [i, file] of brollIdx) {
    const e = edits[i];
    const dur = (e.duration || 2) + 0.5;
    if (e.media === "image") {
      args.push("-loop", "1", "-t", dur.toFixed(2), "-i", file);
    } else {
      args.push("-stream_loop", "-1", "-t", dur.toFixed(2), "-i", file);
    }
  }

  // Base: scale + cover-crop to 720x1280, then zoom expression via crop.
  ...
  const parts = [vf];
  let cur = "base";
  // B-roll is composited ON TOP of the speaker in a framed panel — the talking
  // head is never replaced. "top" = wide panel across the upper third;
  // "pip" = small bordered card in the top-right corner.
  brollIdx.forEach(([editIndex], n) => {
    const inIdx = n + 1;
    const e = edits[editIndex];
    const s = Math.max(0, e.start), dur = e.duration || 2;
    const bv = `br${n}`;
    const isPip = e.mode === "pip";
    const iw = isPip ? 292 : W - 16;
    const ih = isPip ? 392 : 452;
    const fw = isPip ? 300 : W - 8;
    const fh = isPip ? 400 : 460;
    const x = isPip ? W - fw - 12 : 4;
    const y = isPip ? 116 : 96;
    parts.push(
      `[${inIdx}:v]scale=${iw}:${ih}:force_original_aspect_ratio=increase,crop=${iw}:${ih},` +
      `pad=${fw}:${fh}:(ow-iw)/2:(oh-ih)/2:color=white,setsar=1,` +
      `trim=0:${dur.toFixed(2)},setpts=PTS-STARTPTS[${bv}]`
    );
    const out = `v${n}`;
    parts.push(
      `[${cur}][${bv}]overlay=${x}:${y}:enable='between(t,${s.toFixed(2)},${(s + dur).toFixed(2)})'[${out}]`
    );
    cur = out;
  });

  // Burn text cards last so they sit on top of everything (no subtitles — these
  // are graphic keyword highlights only).
  if (assPath) {
    const sub = fontsdir
  ...
  await runFfmpeg(args, "render reel", onProgress);
}
